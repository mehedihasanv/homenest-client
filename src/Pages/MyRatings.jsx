...
//       <h2 className="text-2xl font-bold mb-6 text-center">My Ratings & Reviews</h2>
//       {ratings.length === 0 ? (
//         <p className="text-center text-gray-500">You haven't rated any properties yet.</p>
//       ) : (
//         <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
//           {ratings.map((r) => (
//             <div key={r._id} className="border rounded-lg p-4 shadow hover:shadow-md transition">
//               <img src={r.propertyImage} alt={r.propertyName} className="h-40 w-full object-cover rounded" />
//               <h3 className="text-lg font-semibold mt-2">{r.propertyName}</h3>
//               <p className="text-sm text-gray-600">Location: {r.location}</p>
//               <p className="text-sm text-gray-600">Rated by: {r.userName}</p>
//               <p className="text-sm text-gray-600">Date: {new Date(r.date).toLocaleDateString()}</p>
...
const MyRatings = () => {
  ...
  if (loading || isLoading) {
    return <p className="text-center mt-10 text-gray-600 animate-pulse">⏳ Loading your ratings...</p>;
  }

  return (
    <section className="max-w-6xl mx-auto px-4 py-12 bg-gray-50 min-h-screen">
      <h2 className="text-3xl font-bold mb-10 text-center text-gray-800">🌟 My Ratings & Reviews</h2>
      {ratings.length === 0 ? (
        <p className="text-center text-gray-500 text-lg">You haven't rated any properties yet.</p>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {ratings.map((r) => (
            ...
            >
              ...
              />
              <div className="p-6">
                <h3 className="text-xl font-semibold text-gray-800">{r.propertyName}</h3>
